        ...
        match node {
            ...
            Node::Statement(stmt) => match stmt {
                ...
                StatementNode::Let(_, ident, val) => {
                    ...
                    self.env.borrow_mut().set(ident.into(), val); // irrecoverable error: problem with implementation
                    Ok(Object::Null)
                }
            },
            Node::Expression(expr) => match expr {
                ...
                ExpressionNode::Function(token, args, block) => Ok(Object::Function(token, args, block, self.env.clone())),
                ExpressionNode::FunctionCall(token, function, args) => {
                    ...
                    if let Object::Function(token, params, block, env) = function {
                        ...
                        self.env.borrow_mut().add_new_context();

                        // let new_env = Env::extend(older)

                        for (p, a) in params.into_iter().map(|p| p.into()).zip(args.into_iter()) {
                            self.env.borrow_mut().set(p, a);
                        }

                        let return_val = self.eval_block(block.statements)?;
                        
                        self.env.borrow_mut().get_prev_context();
